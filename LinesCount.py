            ...
            with open(f_path, 'r', encoding='utf-8') as ff:
                ...
                print(f_path + '----' + '总行数=' + str(file_all_count) +
                    '注释行=' + str(file_comment_line_count)+
                      '         去掉空白和注释行之后=' + str(file_all_count-file_blank_line_count-file_comment_line_count))
#统计所有文件的代码行数
def count_all_code(fileList):
    ...
    print('该路径下符合扩展名要求的文件总行数=' + str(all_count) +
                    '注释行=' + str(comment_line_count)+ '空白行=' + str(blank_line_count)+' ,忽略空白行和注释行=' + str(all_count-blank_line_count-comment_line_count))

if __name__ == '__main__':
    code_path = input("Please enter code path: ")
    print("You entered " + str(code_path))
    code_type=input("Please enter code type: ")
    print("You entered " + str(code_type))
    fileList = []
    # count_code(r'E:\code\python\EmailSpider\tools', '.py', fileList)
